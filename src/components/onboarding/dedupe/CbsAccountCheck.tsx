...
import { postCBSAccountCheck, putEkycTemp } from '@config/urls.config'
import { ICbsAccouontCheck, ITempDataRes } from '@interfaces/onboarding.interface'
...
export default function CbsAccountCheck() {
  ...
  const { execute: executeAccountCheck } = useAPIWithToken<ICbsAccouontCheck>()
    ...
    if (ok) {
      try {
        ...
        if (response?.data.result) {
          toast.error(response?.data.details.responseMessage)
          return
        }
