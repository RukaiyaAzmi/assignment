...
import {
  postChannelWiseEkycCountWithFilter,
  postChannelWiseCountReport,
  postChannelWiseEkycByMonth,
  postChannelWiseVerificationByDay,
...
interface StatisticsAbsProps {
  label?: boolean
}

export default function StatisticsAbs({ label = true }: StatisticsAbsProps): JSX.Element {
  ...
  const getEkycCount = async () => {
    try {
      const resSelf = await execute({
        method: 'POST',
        url: postChannelWiseEkycCountWithFilter,
        data: {
  ...
  const count = async () => {
    try {
      const res = await executeCount({
        method: 'POST',
        url: postChannelWiseCountReport,
        data: {
          channelCode: 'ABS',
        },
      })
      ...
      const chart = await executeChart({
        method: 'POST',
        url: postChannelWiseEkycByMonth,
        data: {
          year: year,
          channelCode: 'ABS',
        },
  ...
  const ekycYearChart = async (year: string) => {
    try {
      const chart = await executeChart({
        method: 'POST',
        url: postChannelWiseEkycByMonth,
        data: { year: year, channelCode: 'ABS' },
      })
  ...
  const barChart = async (year: number, month: number) => {
    try {
      const chart = await executeBarChart({
        method: 'POST',
        url: postChannelWiseVerificationByDay,
        data: { month: month, year: year, channelCode: 'ABS' },
      })
  ...
  return (
    <>
      <div className={`container m-auto overflow-x-auto ${label ? 'mt-8' : ''}`}>
        {label && <h1 className="flex justify-center text-2xl text-gray-900 font-bold ">Statistics for E-KYC (ABS)</h1>}
        <div className="flex flex-col w-full rounded-lg py-8 gap-y-3 ">
          <div
            className={`overflow-hidden bg-white rounded-lg shadow-lg h-full p-4 text-gray-900 max-w-full sm:p-8 ${
              label ? 'border-t-4 border-green-500' : ''
            } `}
          >
            <h6 className="text-lg dark:text-white bg-green-100 py-2 my-5 text-center">Search E-KYC</h6>
            ...
            <div>
              <div className="flex flex-col gap-4 lg:flex-row lg:w-full ">
                ...
                <button
                  type="submit"
                  className="flex justify-center item-center gap-1 lg:w-1/12 py-3 mb-5 mt-8  px-4 bg-green-400 rounded-lg hover:bg-green-500 text-white font-semibold"
                  onClick={() => ekycCount()}
                  value="Find"
                >
                  <span className="mt-1">
                ...
                <div className="flex flex-col lg:w-2/3 mb-5 py-5 rounded-lg bg-green-200 items-center justify-center">
              ...
              <div className="mt-0 border-b-2 border-green-700 pt-3"></div>

              <div className="flex flex-col justify-center items-center gap-4 lg:w-full lg:flex-row pt-7">
                <div className="flex flex-col w-2/3 h-40 rounded-lg gap-y-3 pt-5 bg-green-200 items-center justify-center">
              ...
              <div className="mt-2 border-b-2 lg:m-auto border-green-700 pt-7"></div>
              <div className="flex flex-col gap-4 lg:flex-row w-full pt-5">
                <div className="lg:w-1/2">
                  <h6 className="text-lg dark:text-white bg-green-100 my-2 py-3 text-center">E-KYC in {year}</h6>
                  <hr />
            ...
            <div className="mt-2 border-b-2 m-auto border-green-700 pt-5"></div>
            <div className="w-full">
              <h6 className="text-lg dark:text-white bg-green-100 py-2 mt-7 text-center">Total Verification</h6>
              <SelectBox
                id="month"
                name="month"
            ...
            <div className="flex flex-col justify-center items-center mt-10 gap-4 mx-8 mb-8 lg:flex-row">
              <div className="w-full lg:w-1/2 flex flex-col justify-center items-center shadow-md rounded-md text-center p-4">
                <div className="w-full">
                  <h6 className="text-lg dark:text-white bg-green-100 py-2 mt-4 text-center">Onboarding Ratio</h6>
                </div>
              ...
              <div className="w-full lg:w-1/2 flex flex-col justify-center items-center shadow-md rounded-md text-center p-4">
                <div className="w-full">
                  <h6 className="text-lg dark:text-white bg-green-100 py-2 mt-4 text-center">Verification Ratio</h6>
                </div>
                <div className=" w-56 h-56 my-8">
                  <DoughnutChart
