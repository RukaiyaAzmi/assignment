...
import { postAdminReport } from '@config/urls.config'
...
export default function AdminReport() {
  ...
  const [adminReport, setShowReportDetails] = useState<IEkyc>()
  ...
  const fetchAdminList = async () => {
    try {
      ...
      const res = await executeAdminReport({
        method: 'POST',
        url: `${postAdminReport}/${currentPage}`,
      })
  ...
  const handleShowDetailsModal = async (id: string) => {
    try {
      ...
      console.log(resModalDetails?.data)
      ...
      <div className="container m-auto mt-8 overflow-x-auto w-full">
        ...
        >
          ...
          <Modal.Body className=" bg-slate-100">
            {adminReport && <AdminReportView adminReport={adminReport} />}
          </Modal.Body>
